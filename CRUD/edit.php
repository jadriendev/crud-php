...
$id = $_GET['id'];
$row = $conn->query("SELECT * FROM tblcrud WHERE id=$id")->fetch_assoc();
$showSuccess = false;
...
if(isset($_POST['update']))
    {
        ...
        if($conn->query("UPDATE tblcrud SET name = '$name', email = '$email' WHERE id = $id"))
        {
            $showSuccess = true;
            $row = $conn->query("SELECT * FROM tblcrud WHERE id = $id")->fetch_assoc();
        }
    ...
    <div class="min-h-screen flex items-center justify-center">
        <main class="w-full max-w-md">
            <form method="POST" class="bg-white shadow-md px-6 py-6 flex flex-col items-center justify-center gap-3 rounded-lg text-gray-600">
                <h1 class="text-3xl font-bold tracking-wide">Edit User</h1>
                ...
                <div class="relative w-full">
                    ...
                    <input type="text" name="name" value="<?= $row['name']; ?>" class="border border-1 border-gray-300 p-3 pl-10 rounded w-full text-sm">
                </div>
                
                <div class="relative w-full">
                    <i class="fa-solid fa-envelope text-xl text-gray-500 absolute left-3 top-1/2 -translate-y-1/2"></i>
                    <input type="text" name="email" value="<?= $row['email']; ?>" class="border border-1 border-gray-300 p-3 pl-10 rounded w-full text-sm">
                </div>
                ...
                <div class="flex items-center w-full gap-2">
                    ...
                    <input type="submit" name="update"  class="tracking-wide bg-blue-600 text-white p-3 w-full rounded hover:bg-blue-700 transition text-sm font-medium cursor-pointer">
                </div>
            </form>
        </main>
    </div>
